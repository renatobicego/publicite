...
import {
  ...
  UserRelationModel,
} from '../infrastructure/schemas/user.relation.schema';
...
import {
  insertSection,
  insertUserMagazine,
} from 'src/contexts/module_magazine/test/models/db/insert.group.magazine';
import {
...
describe('UserService - Make relation between two users', () => {
  ...
  beforeAll(async () => {
    const moduleRef: TestingModule =
      await mapModuleTesting.get('make-relation')();
    ...
    userRelationModel = moduleRef.get<Model<UserRelationDocument>>(
      getModelToken(UserRelationModel.modelName),
    );
    postModel = moduleRef.get<Model<PostDocument>>(
      getModelToken(PostModel.modelName),
    );
    userMagazineModel = moduleRef.get<Model<UserMagazineDocument>>(
      getModelToken(UserMagazineModel.modelName),
    );
    magazineSectionModel = moduleRef.get<Model<MagazineSectionDocument>>(
      getModelToken(MagazineSectionModel.modelName),
    );
    await createPersonalUser(userModel, {
      _id: userProfileId,
      userRelations: [userRelationId],
  ...
  it('GET profile by external user -Should return PostArray and MagazineArray length 1 RelationShip -> friends & Post visibility -> friends Magazine visibility -> friends', async () => {
    await insertPostGood(postModel, {
      _id: postId,
      author: userProfileId.toString(),
      visibility: { post: 'friends', socialMedia: 'public' },
    });
    await insertSection(magazineSectionModel, {
      _id: sectionId,
      posts: [postId],
    ...
    const user = await userService.findProfileUserByExternalUserById(
      userProfileId.toString(),
      userRequestId.toString(),
    );
  ...
  it('GET profile by external user -Should return PostArray and MagazineArray length 1 relationShip -> friends & Post visibility -> contacts Magazine visibility -> contacts ', async () => {
    await insertPostGood(postModel, {
      _id: postId,
      author: userProfileId.toString(),
      visibility: { post: 'contacts', socialMedia: 'public' },
    });
    await insertSection(magazineSectionModel, {
      _id: sectionId,
      posts: [postId],
  ...
  it('GET profile by external user -Should return PostArray and MagazineArray length 1 relationShip -> topfriends & Post visibility -> topfriends Magazine visibility -> topfriends ', async () => {
    await insertPostGood(postModel, {
      _id: postId,
      author: userProfileId.toString(),
      visibility: { post: 'topfriends', socialMedia: 'public' },
    });
    await insertSection(magazineSectionModel, {
      _id: sectionId,
      posts: [postId],
  ...
  it('GET profile by external user -Should return PostArray and MagazineArray length 0, they dont have relationShip ', async () => {
    await insertPostGood(postModel, {
      _id: postId,
      author: userProfileId.toString(),
      visibility: { post: 'topfriends', socialMedia: 'public' },
  ...
  it('GET profile by external user -Should return PostArray and MagazineArray length 1, they dont have relationShip but, author has items in public ', async () => {
    await insertPostGood(postModel, {
      _id: postId,
      author: userProfileId.toString(),
      visibility: { post: 'public', socialMedia: 'public' },
    });
    await insertSection(magazineSectionModel, {
      _id: sectionId,
      posts: [postId],
