...
export class ChatbotResolver {
  ...
  async createChatSession(
    @Args('createSessionRequest', { type: () => CreateSessionRequest, nullable: true })
    createSessionRequest?: CreateSessionRequest,
    @Context() context?: { req: CustomContextRequestInterface },
  ): Promise<ChatSessionResponse> {
    try {
      const userId = createSessionRequest?.userId || context?.req?.userRequestId;
  ...
  @Mutation(() => SendMessageResponse, {
    nullable: false,
    description: 'Enviar un mensaje al chatbot (crea la sesión automáticamente)',
  })
  ...
  async getChatSessionHistory(
    ...
    try {
      return await this.chatbotAdapter.getSessionHistory(sessionId, limit, page);
    } catch (error: any) {
      throw error;
    }
  }
  ...
  @Mutation(() => Boolean, {
    nullable: false,
    description: 'Eliminar una sesión de chat completa (sesión + todos los mensajes)',
  })
