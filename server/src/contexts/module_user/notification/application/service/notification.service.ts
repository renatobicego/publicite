  ...
  implements NotificationHandlerServiceInterface, NotificationServiceInterface
{
  ...
  async deleteNotificationById(
    ...
    try {
      ...
      const isTheNotificationValidToDelete =
        checkIfNotificationIsValidToDelete(event);
      if (!isTheNotificationValidToDelete)
        throw new BadRequestException('Notification not valid to delete');
  ...
  async deleteAccount(id: string): Promise<void> {
    try {
      ...
      this.logger.log(
        `Successfully deleted notifications of user with id: ${id}`,
      );
    } catch (error: any) {
      throw error;
    }
  ...
  async handlePushSubscriptionNotification(
    paymentDataFromMeli: PaymentDataFromMeli,
  ): Promise<void> {
    try {
      ...
      const notificationPayment = factory.createNotification(
        typeOfNotification.payment_notifications,
        notification,
      );

      return await this.notificationSubscriptionService.createNotificationPaymentAndSendToUser(
        notificationPayment as NotificationPayment,
      );
    } catch (error: any) {
      throw error;
    }
  ...
  async handleSubscriptionNotification(
    ...
    try {
      ...
      const factory = NotificationFactory.getInstance(this.logger);
      const notificationSubscription = factory.createNotification(
        typeOfNotification.subscription_notifications,
        notification,
      );
      return await this.notificationSubscriptionService.createNotificationSubscriptionAndSendToUser(
  ...
  async handleMagazineNotification(notification: any): Promise<any> {
    try {
      const factory = NotificationFactory.getInstance(this.logger);
      const notificationMagazine = factory.createNotification(
        typeOfNotification.magazine_notifications,
        notification,
      );
      const event = notificationMagazine.getEvent;
      if (event === notification_magazine_new_user_invited)
        await this.isThisNotificationDuplicate(
          notificationMagazine.getNotificationEntityId,
        );
      await this.notificationMagazineService.saveNotificationMagazineAndSentToUser(
        notificationMagazine as NotificationMagazine,
      );
    } catch (error: any) {
      throw error;
    }
  ...
  async handleShareNotification(notification: any): Promise<void> {
    try {
      const factory = NotificationFactory.getInstance(this.logger);
      const notificationGroup = factory.createNotification(
        typeOfNotification.share_notifications,
        notification,
      );
      await this.isThisNotificationDuplicate(
        notificationGroup.getNotificationEntityId,
      );
      await this.notificationShareService.createNotificationShareAndSendToUser(
        notificationGroup as NotificationShare,
      );
    } catch (error: any) {
      throw error;
    }
  ...
  async handleGroupNotification(notification: any): Promise<void> {
    try {
      const factory = NotificationFactory.getInstance(this.logger);
      const notificationGroup = factory.createNotification(
        typeOfNotification.group_notifications,
        notification,
      );
      const event = notificationGroup.getEvent;
      if (event === notification_group_new_user_invited)
        await this.isThisNotificationDuplicate(
          notificationGroup.getNotificationEntityId,
        );
      await this.notificationGroupService.saveNotificationGroupAndSentToUserAndGroup(
        notificationGroup as NotificationGroup,
      );
    } catch (error: any) {
      throw error;
    }
  ...
  async handlePostNotification(notification: any): Promise<void> {
    try {
      const notificationPostType =
        notification.frontData.postActivity.notificationPostType;
      ...
      if (
        notificationPost.getPostNotificationType == NotificationPostType.comment
      ) {
        return await this.notificationPostService.saveNotificationPostCommentAndSendToUser(
          notificationPost,
        );
      } else if (
        notificationPost.getPostNotificationType ==
        NotificationPostType.reaction
      ) {
        return await this.notificationPostService.saveNotificationPostReactionAndSendToUser(
          notificationPost,
        );
      } else if (
        notificationPost.getPostNotificationType ==
        ...
        return await this.notificationPostService.saveNotificationPostResponseAndSendToUser(
          notificationPost,
        );
      } else {
  ...
  async handleUserNotification(notification: any): Promise<void> {
    try {
      const factory = NotificationFactory.getInstance(this.logger);
      const notificationUser = factory.createNotification(
        typeOfNotification.user_notifications,
        notification,
      );
      ...
      if (
        ...
        await this.isThisNotificationDuplicate(
          notificationUser.getNotificationEntityId,
        );
      }
      await this.notificationUserService.saveNotificationUserAndSentToUser(
        notificationUser as NotificationUser,
      );
    } catch (error: any) {
      throw error;
    }
  ...
  async handleContactSellerNotification(notification: any): Promise<void> {
    try {
      const factory = NotificationFactory.getInstance(this.logger);
      const notificationContactSeller = factory.createNotification(
        typeOfNotification.contact_seller_notifications,
        notification,
      );
      const event = notificationContactSeller.getEvent;
      if (event === notification_new_contact) {
        await this.isThisNotificationDuplicate(
          notificationContactSeller.getNotificationEntityId,
        );
      }
      return await this.notificationContactSellerService.createNotificationContactSellerAndSendToUser(
        notificationContactSeller,
      );
    } catch (error: any) {
      throw error;
    }
  ...
  async handlePostCalificationNotification(notification: any): Promise<void> {
    try {
      const factory = NotificationFactory.getInstance(this.logger);
      const notificationRequestCalification = factory.createNotification(
        typeOfNotification.post_calification_notifications,
        notification,
      );
      ...
      if (event === notification_new_calification_request) {
        await this.isThisNotificationDuplicate(
          notificationRequestCalification.getNotificationEntityId,
        );
        return await this.notificationRequestCalificationService.createNotificatioRequestCalificationAndSendToUser(
          notificationRequestCalification as NotificationPostCalification,
        );
      } else if (event === notification_new_calification_response) {
        await this.isThisNotificationDuplicate(
          notificationRequestCalification.getNotificationEntityId,
        );
        return await this.notificationRequestCalificationService.createNotificationResponseCalificationAndSendToUser(
          notificationRequestCalification as NotificationPostCalification,
        );
      } else {
        throw new Error(
          'Event is not supported in handleRequestCalificationNotification',
        );
      }
    } catch (error: any) {
      throw error;
  ...
  async isThisNotificationDuplicate(
    ...
    try {
      ...
      const isDuplicate =
        await this.notificationRepository.isThisNotificationDuplicate(
