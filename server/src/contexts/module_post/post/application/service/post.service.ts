...
import {
  makeUserRelationMapWithoutHierarchy,
  makeUserRelationHierarchyMap,
} from 'src/contexts/module_shared/utils/functions/makeUserRelationHierarchyMap';
...
export class PostService implements PostServiceInterface {
  ...
  async activateOrDeactivatePost(
    ...
    try {
      if (activate) {
        const isAllowedToActivate =
          await this.userService.isThisUserAllowedToPost(
            userRequestId,
            postBehaviourType,
          );
        ...
        if (!isAllowedToActivate) {
          throw new BadRequestException(
            'No es posible activar el post, agostaste el limite de posts según tu plan',
          );
        }
  ...
  async create(post: PostRequest): Promise<any> {
    ...
    if (!author || !postBehaviourType)
      throw new Error('Author or postBehaviourType is required');

    const session = await this.connection.startSession();
    let newPostId: string | null;
    try {
      const newPostIdId = await session.withTransaction(async () => {
        const isThisUserAllowedToPost =
          await this.userService.isThisUserAllowedToPost(
            author,
            postBehaviourType,
          );

        if (!isThisUserAllowedToPost)
          throw new BadRequestException(
            'No es posible crear el post, agostaste el limite de posts según tu plan',
          );
  ...
  async desactivatePostByUserId(userId: string): Promise<void> {
    try {
      ...
      const {
        ...
        totalLibrePostLimit,
      } = await this.userService.getPostAndLimitsFromUserByUserId(userId);
      ...
      this.logger.log(
        `totalLibresExceded: ${totalLibresExceded}, totalAgendaExceded: ${totalAgendaExceded}`,
      );
  ...
  async deleteCommentById(
    ...
    try {
      await this.postRepository.deleteCommentById(
        id,
        userRequestId,
        isAuthorOfPost,
        isReply,
  ...
  async findMatchPost(postType: string, searchTerm: string): Promise<void> {
    try {
      const termsWithOutAccentsAndEmojis =
        removeAccents_removeEmojisAndToLowerCase(searchTerm);
      const searchTermNormalized = checkStopWordsAndReturnSearchQuery(
        termsWithOutAccentsAndEmojis,
        SearchType.post,
      );
      if (searchTermNormalized === null) return;
      return await this.postRepository.findMatchPost(
        postType,
  ...
  async findPostOfGroupMembers(
    ...
    try {
      return await this.postRepository.findPostOfGroupMembers(
        membersId,
        conditionsOfSearch,
        userLocation,
        limit,
  ...
  async findPostByIdAndCategoryPostsRecomended(id: string): Promise<any> {
    try {
      return await this.postRepository.findPostByIdAndCategoryPostsRecomended(
        id,
      );
    } catch (error: any) {
      throw error;
  ...
  async makeUserRelationMap(
    userRequestId: string,
    visibility: Visibility_Of_Find,
  ): Promise<any> {
    const userActiveRelation =
      await this.userService.getActiveRelationOfUser(userRequestId);
    ...
    if (visibility === Visibility_Of_Find.hierarchy) {
      ...
      return makeUserRelationMapWithoutHierarchy(
        userActiveRelation,
        userRequestId,
        visibility,
      );
  ...
  async makeReactionSchemaAndSetReactionToPost(
    ...
    try {
      ...
      return await this.postRepository.makeReactionSchemaAndSetReactionToPost(
        postId,
        reaction,
        session,
      );
      ...
      this.logger.error(
        'An error was ocurred setting reactions to posts id: ' + postId,
      );
      throw error;
    }
  }

  async makeResponseAndPutResponseInComment(
    author: string,
    commentId: string,
    response: string,
    session: any,
    ...
    try {
      ...
      const commentResponse = await this.postRepository.savePostComment(
        newResponse,
        session,
      );
      ...
      if (!commentResponseId)
        throw new Error(
          'Error al crear la respuesta in makeCommentSchemaAndPutCommentInPost',
        );
      await this.postRepository.setResponseOnComment(
        commentId,
        commentResponseId,
        session,
      );
  ...
  async updateEndDateFromPostById(
    ...
    try {
      ...
      return await this.postRepository.updateEndDateFromPostById(
        postId,
        userRequestId,
        newDate,
      );
  ...
  async updateBehaviourType(
    ...
    try {
      ...
      const isAllawedToChange = await this.userService.isThisUserAllowedToPost(
        userRequestId,
        postBehaviourType,
      );
      ...
      if (isAllawedToChange) {
        ...
        throw new BadRequestException(
          'You are not allowed to change behaviour_type, please upgrade your plan',
        );
      }
    } catch (error: any) {
      throw error;
  ...
  async updateCommentById(
    ...
    try {
      ...
      return await this.postRepository.updateCommentById(
        id,
        comment,
        userRequestId,
      );
  ...
  async removeReactionFromPost(
    ...
    try {
      ...
      this.logger.error(
        'An error was ocurred removing reaction from post with id: ' + _id,
      );
      throw error;
    }
  }
